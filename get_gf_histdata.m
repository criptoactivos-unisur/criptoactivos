...
% History:  03-Mar-18: Updated link to Google Finance, changed from http://www.google.com/finance/historical to
%                      http://finance.google.com/finance/historical + minor fixes.
...
startdate=[datestr(today,3) '+' datestr(today,7) '%2C+' num2str(str2double(datestr(today,10))-1)];
...
%make URL
%url=['http://finance.google.com/finance/historical?q=' stock '&histperiod' period '&startdate=' startdate '&enddate' enddate '&output=csv']; 
url=['http://finance.google.com/finance/historical?q=' stock '&histperiod' period '&startdate=' startdate '&enddate' enddate '&output=csv']; 
...
if gfstatus
    ...
    if hist_stock_data
        ...
        clear stock_data;
        stock_data(length(stock_data_old.date))=struct;
